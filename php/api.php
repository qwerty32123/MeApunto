...
if(isset($_GET['apicall'])){
    ...
    switch($_GET['apicall']){
        ...
        case 'anadirReserva':
            if(isTheseParametersAvailable(array('dia','pista','id_timeslot','id_usuario','id_partido'))){

                $dia = $_POST['dia'];

                $pista = $_POST['pista'];
                $id_timeslot = $_POST['id_timeslot'];
                $id_usuario = $_POST['id_usuario'];

                //agarramos el id de timeslot
                $id_partido = $_POST['id_partido'];




                //codigo sobre que ya existe el partido
                $stmt = $conn->prepare("INSERT INTO participa (id_partido,idUsuario) VALUES (?, ?)");
                $stmt->bind_Param("ss", $id_partido,$id_usuario);

                $stmt->execute();
                $stmt->store_result();
                $stmt->close();

                $stmts = $conn->prepare("INSERT INTO agenda (dia,timeSlot,id_pista,id_partido,disponible) VALUES (?, ?,?,?,?)");
                $disponible = 1;
                $stmts->bind_Param("sssss", $dia,$id_timeslot,$pista,$id_partido,$disponible);
                ...
                $response['result'] = "reserva realizada correctamente";
                break;
                }else{
                $response['error'] = true;
                $response['result'] = 'required parameters are not available';
                break;
            }

                //codigo si el partido no existe
...
///////// START PISTAS GET
        ...
        case 'get_timeslot_toremove':

            //SELECT timeslot.description FROM agenda INNER JOIN timeslot ON agenda.timeSlot = timeslot.id WHERE agenda.disponible = 0 AND agenda.dia = '2022-02-18' AND agenda.id_pista = 1;
            if(isTheseParametersAvailable(array('dia','pista'))){
                ...
                $stmt = $conn->prepare("SELECT timeslot.id,timeslot.description FROM agenda INNER JOIN timeslot ON agenda.timeSlot = timeslot.id WHERE agenda.disponible = 0 AND agenda.dia = ? AND agenda.id_pista = ?");
                $stmt->bind_param("ss",$dia,$pista);
                $stmt->execute();
                $stmt->bind_result($id,$description);
